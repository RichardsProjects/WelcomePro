...
public class WelcomePro extends JavaPlugin {
	...
	private void startup() throws FileNotFoundException {
		dataFolder = getServer().getPluginManager().getPlugin("WelcomePro")
				.getDataFolder();
		...
		serverNameFile = new File(dataFolder + File.separator
				+ "serverName.txt");
		serverJoinMessageFile = new File(dataFolder + File.separator
				+ "joinMessage.txt");
		serverLeaveMessageFile = new File(dataFolder + File.separator
				+ "leaveMessage.txt");
		messageTypeFile = new File(dataFolder + File.separator
				+ "messageType.txt");
		...
		if (!(dataFolder.exists())) {
			...
			if ((!line1File.exists()) || (!line2File.exists())
					|| (!line3File.exists())) {
				if (!line1File.exists()) {
					try {
						line1File.createNewFile();
	...
	private void loadMotdValues() throws IOException {
		...
		if (messageTypeFile.length() > 0) {
			BufferedReader reader7 = new BufferedReader(new FileReader(
					messageTypeFile));
