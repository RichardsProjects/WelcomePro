...
public class WelcomeProCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2,
		...
		if(args.length == 0) {
			//Show help menu
			sender.sendMessage(ChatColor.BLUE + "--------------- WelcomePro Commands ------------------");
			sender.sendMessage(ChatColor.GOLD + "/wp set <servername|messagetype|line1|line2|line3> <value>");
			sender.sendMessage(ChatColor.GOLD + "/wp version");
		} else {
			if(args[0].equals("set")) {
				if(args.length >= 3) {
					if(args[1].toUpperCase().equals("SERVERNAME") || args[1].toUpperCase().equals("MESSAGETYPE") || args[1].toUpperCase().equals("LINE1") || args[1].toUpperCase().equals("LINE2") || args[1].toUpperCase().equals("LINE3")) {
						if(args[1].toUpperCase().equals("SERVERNAME")) {
							...
							if(serverName.length() > 0) {
								if(!(serverName.length() > 20)) {
									...
									try {
										...
										out.close();
										sender.sendMessage(ChatColor.YELLOW + "Server Name updated to \"" + serverName + "\"");
									...
									sender.sendMessage(ChatColor.YELLOW + "The max length is 20. The name you entered is " + length + " characters.");									
								}
							} else {
							...
							if(input.equals("MESSAGE")) {
								...
								try {
									PrintWriter out = new PrintWriter(WelcomePro.messageTypeFile);
									out.print("MESSAGE");
									out.close();
									sender.sendMessage(ChatColor.YELLOW + "Message Type has been set to message.");
								} catch (FileNotFoundException e) {
								...
								sender.sendMessage(ChatColor.YELLOW + "Message Type has been set to Message of the Day.");
							} else {
								sender.sendMessage(ChatColor.YELLOW + "That is a not a valid message type: Motd, Message");								
							}
							...
							if(line1.length() > 0) {
								if(!(line1.length() > 35)) {
									...
									try {
										PrintWriter out = new PrintWriter(WelcomePro.line1File);
										out.print(line1);
										out.close();
										sender.sendMessage(ChatColor.YELLOW + "Line 1 updated to \"" + line1 + "\"");
									} catch (FileNotFoundException e) {
										sender.sendMessage("There was an error updating the line. Please check the file permissions.");
									}
								} else {
									int length = line1.length();
									sender.sendMessage(ChatColor.YELLOW + "The max length is 35. The name you entered is " + length + " characters.");
								}
							} else {
							...
							if(line2.length() > 0) {
								if(!(line2.length() > 35)) {
									...
									try {
										PrintWriter out = new PrintWriter(WelcomePro.line2File);
										out.print(line2);
										out.close();
										sender.sendMessage(ChatColor.YELLOW + "Line 2 updated to \"" + line2 + "\"");
									} catch (FileNotFoundException e) {
										sender.sendMessage("There was an error updating the line. Please check the file permissions.");
									}
								} else {
							...
							if(line3.length() > 0) {
								if(!(line3.length() > 35)) {
									...
									try {
										PrintWriter out = new PrintWriter(WelcomePro.line3File);
										out.print(line3);
										out.close();
										sender.sendMessage(ChatColor.YELLOW + "Line 3 updated to \"" + line3 + "\"");
									} catch (FileNotFoundException e) {
										sender.sendMessage("There was an error updating the line. Please check the file permissions.");
									}
								} else {
						...
						sender.sendMessage(ChatColor.BLUE + "" + ChatColor.BOLD + "That cannot be set to a value. Possible values:");
						sender.sendMessage("Servername, Messagetype, Line1, Line2, Line3");
					}					
				} else {
				...
				sender.sendMessage(ChatColor.BLUE + "WelcomePro current version: " + this.plugin.getDescription().getVersion());
			}
		}		
		return true;
	}
